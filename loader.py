...
def load_dataset(path):
    ...
    train_set_y_orig = np.array(
        train_dataset["train_labels"][:])  # your train set labels

    val_set_x_orig = np.array(train_dataset["val_img"][:])
    val_set_y_orig = np.array(train_dataset["val_labels"][:])

    test_set_x_orig = np.array(train_dataset["test_img"][:])
    test_set_y_orig = np.array(train_dataset["test_labels"][:])
...
def predict(X, parameters):
    ...
    x = tf.placeholder("float", [150528, 1])
